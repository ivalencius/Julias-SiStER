function SiStER_get_strain_rate(vx,vy,dx,dy,BC)
# [EXX EXY]=SiStER_get_strain_rate(vx,vy,dx,dy,BC)
# computes the deviatoric strain rate: EXX; at normal nodes
# and EXY; at shear nodes; from vx & vy on the eulerian grid()
#
# G.Ito 11/12/15 updated calculations at corner nodes to be
#    consistent with the top & bottom boundary conditions; as those
    ...
    for j=1:Nx   
        ...
        if i==1 && j==1
            ...
            elseif BC.top[2]==3 # open top so set to be the same as adjacent node
                ...
                print("Stopped in get_strain_rate:  EXY for corner elements not coded for BC.top[2]="*string(BC.top[2]));
                halt
            end   
        ...
        elseif i==Ny && j==Nx
            ...
            elseif BC.bot[2]==3 # open bottom so set to be the same as adjacent node
                ...
                print("Stopped in get_strain_rate:  EXY for corner elements not coded for BC.bot[2]="*string(BC.bot[2]));
                halt
            end
            EXY[i,j]=(dvxdy + dvydx)/2;
