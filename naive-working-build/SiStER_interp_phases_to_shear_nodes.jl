...
wm1 = 1 - (dxm.*dym + (ddx-dxm).*dym + (ddy-dym).*dxm)./(ddx.*ddy);
w1 = accumarray[[ones(sum(cell1),dims=1), JCN[cell1]'], wm1, [1, Nx]];
...
wm2 = 1 - (dxm.*dym + (ddx-dxm).*dym + (ddy-dym).*dxm)./(ddx.*ddy);
w2  = accumarray[[ones(sum(cell2),dims=1), JCN[cell2]'], wm2, [1, Nx]];
...
for n = 1:maxPhases
    ...
    temp = (wc1*accumarray[[ones(sum(cell1 & phaseMask),dims=1), JCN[cell1 & phaseMask]'], wm1[phaseMask[cell1]], [1, Nx]]./w1 + ...
        wc2*accumarray[[ones(sum(cell2 & phaseMask),dims=1), JCN[cell2 & phaseMask]'], wm2[phaseMask[cell2]], [1, Nx]]./w2)/...
        (wc1+wc2);
    phasesS[Ny,:,n] = temp;
end
...
dym = ym[cell1] - y[ICN[cell1]];
wm1 = 1 - (dxm.*dym + (ddx-dxm).*dym + (ddy-dym).*dxm)./(ddx.*ddy);
w1 = accumarray[[ICN[cell1]', ones(sum(cell1),dims=1)], wm1, [Ny, 1]];
...
dym = ym[cell2] - y[ICN[cell2]];
wm2 = 1 - (dxm.*dym + (ddx-dxm).*dym + (ddy-dym).*dxm)./(ddx.*ddy);
w2 = accumarray[[ICN[cell2]', ones(sum(cell2),dims=1)], wm2, [Ny, 1]];
...
for n = 1:maxPhases
    ...
    temp = (wc1*accumarray[[ICN[cell1 & phaseMask]', ones(sum(cell1 & phaseMask),dims=1)], wm1[phaseMask[cell1]], [Ny, 1]]./w1 + ...
        wc2*accumarray[[ICN[cell2 & phaseMask]', ones(sum(cell2 & phaseMask),dims=1)], wm2[phaseMask[cell2]], [Ny, 1]]./w2)/...
        (wc1+wc2);
    phasesS[:, 1, n] = temp;
end
